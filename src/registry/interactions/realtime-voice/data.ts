...
export const VOICE_PRESETS: Record<VoicePresetKey, VoicePreset> = {
  ...
  incident: {
    id: 'incident',
    title: 'Model Watch',
    description: 'Inference latency, hallucination drift, and token throughput health',
    modelName: 'GPT-4o Realtime',
    category: 'AI Ops',
    dialogues: [
      {
        id: 'watch-latency',
        listen: "What's the status on the production inference cluster?",
        think: 'Pulling p99 latency and token throughput across active model replicas…',
        speak:
          'Latency spiked to 380ms from a queuing backlog, Adriel. Rerouting requests to the warm standby replica now.',
      },
      {
        id: 'watch-drift',
        listen: 'Is the hallucination rate still trending up on the new checkpoint?',
        think: 'Comparing grounded-response scores against the eval suite from the last six hours…',
        speak: "It's back under threshold. The retrieval reranker fix dropped the hallucination rate to 1.2 percent.",
      },
    ],
  },
  podcast: {
