...
import { IconChevronDown, IconArrowUp, IconPlaneDeparture } from '@tabler/icons-react'
import { useState } from 'react'
import { cn } from '@/registry/lib/utils'
import { Badge } from '@/registry/primitives/badge'
import { Button } from '@/registry/primitives/button'
import { CardFrame, CardFrameFooter } from '@/registry/primitives/card'
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        ...
        >
          <div className={isDelayed ? 'text-warning-foreground' : undefined}>{row.original.departureTime}</div>
          <div
            aria-hidden="true"
          ...
          />
          <div className={cn('text-muted-foreground', isCancelled && 'line-through')}>{row.original.duration}</div>
          <div
            aria-hidden="true"
    ...
    cell: ({ row }) => {
      ...
      return (
        <Badge variant="outline">
          <span aria-hidden="true" className={cn('size-1.5 rounded-full', getStatusColor(status))} />
          {status}
        </Badge>
      )
    },
          ...
          {table.getHeaderGroups().map((headerGroup) => (
            <TableRow className="hover:bg-transparent" key={headerGroup.id}>
              {headerGroup.headers.map((header) => {
                ...
                return (
                  <TableHead key={header.id} style={columnSize ? { width: `${columnSize}px` } : undefined}>
                    {header.isPlaceholder ? null : header.column.getCanSort() ? (
                      ...
                      >
                        ...
                        {{
                          asc: <IconArrowUp aria-hidden="true" className="size-4 shrink-0 opacity-80" />,
                          desc: <IconChevronDown aria-hidden="true" className="size-4 shrink-0 opacity-80" />,
