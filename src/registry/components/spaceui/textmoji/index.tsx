...
function EmojiGlyph({ emoji, source, className }: { emoji: string; source: EmojiSource; className?: string }) {
  ...
  const src = React.useMemo(() => {
    try {
      return resolveEmojiUrl(emoji, { source, type: EmojiType.Flat } as never)
    } catch {
      return ''
    }
  }, [emoji, source])
...
/** Providers with a static style — Telegram has none, see the separate Animoji component. */
export type StaticEmojiSource =
  | typeof EmojiSource.Fluent
  | typeof EmojiSource.Apple
  | typeof EmojiSource.Twemoji
  | typeof EmojiSource.Blobmoji
  | typeof EmojiSource.Noto

export type TextmojiProps = {
  children: string
  /** @default 'fluent' */
...
/** Renders every Unicode emoji in `children` as a static image sized to the surrounding text. */
export function Textmoji({ children, source = EmojiSource.Fluent, className, emojiClassName }: TextmojiProps) {
  return (
