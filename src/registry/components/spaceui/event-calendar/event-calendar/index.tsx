...
import {
  ...
  type WeekStartsOn,
} from '@/registry/components/spaceui/event-calendar/event-calendar-lib'
import type {
  CalendarEvent,
  CalendarView,
...
} from '@/registry/components/spaceui/event-calendar/event-calendar-types'
...
type EventOrder<TData> = (a: EventCalendarOccurrence<TData>, b: EventCalendarOccurrence<TData>) => number
function priorityEventOrder<TData>(getEventPriority: (event: CalendarEvent<TData>) => number): EventOrder<TData> {
...
interface EventCalendarInternals<TData = unknown> {
  ...
  applyProposedUpdate(update: EventCalendarProposedUpdate<TData>, extraPatch?: Partial<CalendarEvent<TData>>): boolean
...
function createEventCalendarStore<TData>(initial: UseEventCalendarStateOptions<TData>): EventCalendarStore<TData> {
  let options = initial
  ...
  const mergedInteractions = (input: Partial<EventCalendarInteractions>): EventCalendarInteractions => {
  ...
  const getState = (): EventCalendarState<TData> => {
    ...
    snapshot = {
      ...
      interactions: options.interactions ? mergedInteractions(options.interactions) : internal.interactions,
  ...
  type ControlledKey = 'view' | 'date' | 'dayCount' | 'events' | 'selection' | 'interactions' | 'viewSettings'
  ...
  const applyProposedUpdate = (
    ...
    const oldKey = update.occurrence?.key ?? (stored ? `${stored.id}::${stored.start.toISOString()}` : null)
    if (oldKey) {
  ...
  const api: EventCalendarApi<TData> = {
    ...
    updateEvent(id, patch) {
      ...
      const timingChanged = patch.start !== undefined || patch.end !== undefined || patch.allDay !== undefined
...
function useEventCalendarSelector<TData = unknown, TSelected = unknown>(
  ...
  if (!instance) {
    throw new Error('useEventCalendarSelector needs an <EventCalendar> ancestor or an explicit `calendar` option')
  }
...
const midnightTicker = (() => {
  const zones = new Map<string, { listeners: Set<() => void>; timer: ReturnType<typeof setTimeout> | null }>()
  let version = 0
...
function useMidnightTick(timeZone: string): number {
  const subscribe = useCallback((listener: () => void) => midnightTicker.subscribe(timeZone, listener), [timeZone])
...
function useEventCalendarOccurrences<TData = unknown>(
  ...
  return useEventCalendarSelector<TData, EventCalendarOccurrence<TData>[]>(() => instance.api.getOccurrences(range), {
    calendar: instance,
...
function useEventCalendarDay<TData = unknown>(
  ...
  const bucket = useEventCalendarSelector<TData, EventCalendarDayBucket<TData>>(
    () => instance.internals.getIndex().byDay.get(dayKey) ?? (EMPTY_BUCKET as EventCalendarDayBucket<TData>),
    {
      calendar: instance,
  ...
  const activeRange = useEventCalendarSelector<TData, EventCalendarDateRange>((state) => state.activeRange, {
    calendar: instance,
...
function useEventCalendarWeek<TData = unknown>(
  ...
  const row = useEventCalendarSelector<TData, { bars: EventCalendarSegment<TData>[]; rowStart: Date | null }>(
    () => {
    ...
    },
    {
      ...
      isEqual: (a, b) =>
        ...
        (a.bars === b.bars || (a.bars.length === b.bars.length && a.bars.every((segment, i) => segment === b.bars[i]))),
    },
  )
...
function useEventCalendarViewSettings(): {
  ...
  return {
    ...
    effective: {
      ...
      offDays: viewSettings.offDays ?? (viewConfig.offDays !== undefined && viewConfig.offDays !== false),
    },
  }
}
...
interface EventCalendarViewConfig<TData = unknown> {
  ...
  renderAllDaySection?: (props: { days: Date[]; segments: EventCalendarSegment<TData>[] }) => ReactNode
  renderMoreIndicator?: (props: { day: Date; count: number; segments: EventCalendarSegment<TData>[] }) => ReactNode
  /**
   ...
   */
  renderMoreContent?: (props: { day: Date; segments: EventCalendarSegment<TData>[]; close: () => void }) => ReactNode
  /**
...
function EventCalendar<TData = unknown>({
  ...
  const registerRoot = useCallback((el: HTMLElement | null) => instance.internals.setRootEl(el), [instance])
