...
import { useDataGrid } from '@/registry/components/spaceui/data-grid/data-grid'
...
const headerCellSpacingVariants = ({ size }: { size?: 'dense' | 'default' }) => (size === 'dense' ? 'px-2 h-8' : 'px-3')
...
function getDataGridResizeEventClientX(
  ...
  if (isDataGridTouchEvent(event)) {
    if (typeof touchIdentifier === 'number') {
      return findTouchClientX(event.touches, touchIdentifier) ?? findTouchClientX(event.changedTouches, touchIdentifier)
    }
...
function getDataGridTableOrderedVisibleCells<TData>(row: Row<TData>) {
  return [...row.getLeftVisibleCells(), ...row.getCenterVisibleCells(), ...row.getRightVisibleCells()] as Cell<
    TData,
    unknown
  >[]
}
...
function DataGridTableFillCol() {
  ...
  return <col data-slot="data-grid-table-fill-col" style={{ width: 'var(--data-grid-fill-size, 0px)' }} />
}
  ...
  return (
    <th
      ...
      className={cn(
        ...
        props.tableLayout?.columnsResizable && column.getCanResize() && isLastVisibleColumn && 'pe-8',
...
function DataGridTableHeadRowCellResize<TData>({ header }: { header: Header<TData, unknown> }) {
  ...
  const handleMouseDown = (event: ReactMouseEvent<HTMLDivElement>) => {
    ...
    if (isResizeModeOnEnd) {
      stopResizeSessionRef.current?.()
      stopResizeSessionRef.current = startDataGridColumnResizeOnEnd(event, header, table)
      return
    }
...
function DataGridTableResizeIndicator({ viewportNodeRef }: { viewportNodeRef: RefObject<HTMLDivElement | null> }) {
  const { props, table } = useDataGrid()
  ...
  const isActive = !!(props.tableLayout?.columnsResizable && resizeMode === 'onEnd' && resizingColumnId)
  ...
  return (
    <div ref={indicatorRef} aria-hidden="true" className="pointer-events-none absolute inset-y-0 z-20">
...
function DataGridTableBodyRowSkeletonCell<TData>({ children, column }: { children: ReactNode; column: Column<TData> }) {
  ...
  return (
    <td
      style={props.tableLayout?.columnsResizable ? { width: `calc(var(--col-${column.id}-size) * 1px)` } : undefined}
      className={cn(
        'align-middle',
        bodyCellSpacing,
        props.tableLayout?.cellBorder && 'border-e',
...
function DataGridTableBodyRowExpandded<TData>({ row }: { row: Row<TData> }) {
  ...
  return (
    <tr className={cn(props.tableLayout?.rowBorder && bodyRowBottomBorderClasses)}>
      <td colSpan={getDataGridTableOrderedVisibleCells(row).length + (props.tableLayout?.columnsResizable ? 1 : 0)}>
        {table
          .getAllColumns()
...
function DataGridTableRenderedRow<TData>({
  ...
  return (
    <Fragment>
      <DataGridTableBodyRow row={row} pinnedBoundary={pinnedBoundary} rowRef={rowRef} dataIndex={rowIndex}>
...
function DataGridTableEmpty() {
  ...
  const visibleColumnCount =
    getDataGridTableOrderedVisibleColumns(table).length + (props.tableLayout?.columnsResizable ? 1 : 0)

  return (
    <tr>
      <td colSpan={Math.max(visibleColumnCount, 1)} className="text-muted-foreground py-6 text-center text-sm">
...
function DataGridTableRowSelect<TData>({ row }: { row: Row<TData> }) {
  return (
    <>
      <div
        className={cn('bg-primary absolute inset-s-0 top-0 bottom-0 hidden w-[2px]', row.getIsSelected() && 'block')}
      ></div>
      <Checkbox
...
function DataGridTableBodyRows<TData>({ table }: { table: Table<TData> }) {
  ...
  if (isLoading && props.loadingMode === 'spinner') {
    return (
      <tr>
        <td
          colSpan={table.getVisibleFlatColumns().length + (props.tableLayout?.columnsResizable ? 1 : 0)}
          className="p-8"
        >
          ...
          {mergedHeaderGroups.map((headerGroup) => {
            return (
              <DataGridTableHeadRow key={headerGroup.id} rowId={headerGroup.id}>
                ...
                {props.tableLayout?.columnsResizable && hasRightPinnedColumns ? <DataGridTableFillHeadCell /> : null}
                {headerGroup.headers
                  .filter((header) => header.column.getIsPinned() === 'right')
                  .map((header) => {
                    const { column } = header
  ...
  return (
    <DataGridTableViewport>
      <DataGridTableBase>
        ...
        {renderHeader && (props.tableLayout?.stripped || !props.tableLayout?.rowBorder) && <DataGridTableRowSpacer />}
