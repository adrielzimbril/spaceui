...
import { IconCornerUpLeft, IconCornerUpRight, IconChevronLeft, IconChevronRight, IconX } from '@tabler/icons-react'
...
import { Calendar, CalendarDayButton } from '@/registry/primitives/calendar'
...
export function useDateSelector({
  ...
  const [periodType, setPeriodType] = useState<DateSelectorPeriodType>(value?.period || validDefaultPeriodType)
      ...
      {showNavigationButtons && value === 'day' && calendarMonth && onMonthChange && (
        <div className="flex items-center">
          {(() => {
            ...
            return (
              <Button variant="ghost" className="size-8.5" onClick={() => onMonthChange(new Date())} title={i18n.today}>
                {isFuture ? <IconCornerUpLeft /> : <IconCornerUpRight />}
              </Button>
            )
          })()}
          <Button variant="ghost" className="size-8.5" onClick={() => onMonthChange(subMonths(calendarMonth, 1))}>
            <IconChevronLeft className="size-4" />
          </Button>
          <Button variant="ghost" className="size-8.5" onClick={() => onMonthChange(addMonths(calendarMonth, 1))}>
