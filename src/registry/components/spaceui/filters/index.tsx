...
import { IconAlertCircle, IconX, IconCheck, IconPlus } from '@tabler/icons-react'
...
import { ButtonGroup, ButtonGroupText } from '@/registry/primitives/group'
import {
  Menu,
  MenuCheckboxItem,
  MenuPopup,
...
function FilterRemoveButton({ className, icon = <IconX />, ...props }: FilterRemoveButtonProps) {
  const context = useFilterContext()
...
function FilterOperatorDropdown<T = unknown>({ field, operator, values, onChange }: FilterOperatorDropdownProps<T>) {
  const context = useFilterContext()
  const operators = useMemo(() => getOperatorsForField(field, values, context.i18n), [field, values, context.i18n])
    ...
    <Menu>
      ...
      <MenuPopup align="start" className="w-fit min-w-fit">
        {operators.map((op) => (
          ...
          >
            <span>{op.label}</span>
            <IconCheck className={cn('text-primary ms-auto', op.value === operator ? 'opacity-100' : 'opacity-0')} />
          </MenuItem>
        ))}
      </MenuPopup>
    </Menu>
...
function SelectOptionsPopover<T = unknown>({
  ...
  const filteredUnselectedOptions = isAsync
    ? unselectedOptions
    : unselectedOptions.filter((opt) => opt.label.toLowerCase().includes(searchInput.toLowerCase()))
  ...
  return (
    ...
    >
          ...
          <Button variant="outline" size={context.size}>
            <div className="flex items-center gap-1.5">
              {field.customValueRenderer ? (
                field.customValueRenderer(values, isAsync ? resolveSelected(values) : field.options || [])
              ) : (
                <>
      ...
      />
      <MenuPopup align="start" className={cn('w-[200px] px-0', field.className)}>
        {renderMenuContent()}
      </MenuPopup>
    </Menu>
  )
...
function FilterSubmenuContent<T = unknown>({
  ...
  const filteredOptions = useMemo(() => {
    ...
    if (isAsync) {
      ...
      return [...resolveSelected(currentValues), ...resolvedOptions.filter((option) => !selectedSet.has(option.value))]
    }
    return (
      ...
      <div className="relative flex max-h-full">
        ...
        >
          {isAsync && loading && filteredOptions.length === 0 ? (
            ...
            <ScrollArea className="size-full min-h-0 **:data-[slot=scroll-area-scrollbar]:m-0 [&_[data-slot=scroll-area-viewport]]:h-full [&_[data-slot=scroll-area-viewport]]:overscroll-contain">
              <MenuGroup>{filteredOptions.map((option, index) => renderOptionItem(option, index))}</MenuGroup>
            </ScrollArea>
          )}
        </div>
      </div>
      ...
      <div className={cn(filtersContainerVariants({ variant, size }), className)}>
        {selectableFields.length > 0 && (
          ...
          >
            ...
            <MenuPopup className={cn('w-[220px]', menuPopupClassName)} align="start">
              {showSearchInput && (
                <>
                    ...
                    <Input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'ArrowDown') {
                          ...
                          const hasSubMenu =
                            field && (field.type === 'select' || field.type === 'multiselect') && fieldHasOptions(field)
                        ...
                        if (hasSubMenu) {
                          ...
                          return (
                            ...
                            >
                              ...
                              <MenuSubPopup className="w-[200px]" side="right">
                                    ...
                                    if (isMultiSelect) {
                                      ...
                                      if (sessionFilter) {
                                        if (nextValues.length === 0) {
                                          ...
                                          onChange(
                                            filters.map((f) =>
                                              f.id === sessionFilter.id ? { ...f, values: nextValues } : f,
                                            ),
                                          )
                                        }
                                      } else {
