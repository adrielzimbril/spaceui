...
function Sortable<T>({
  ...
  return (
    <SortableInternalContext.Provider value={contextValue}>
      ...
      >
        <SortableContext items={itemIds} strategy={STRATEGY_MAP[strategy] ?? verticalListSortingStrategy}>
          {useRender({
            defaultTagName: 'div',
            render,
...
function SortableItemHandle({ className, render, cursor = true, ...props }: SortableItemHandleProps) {
