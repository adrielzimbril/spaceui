...
export function PageActions({
  ...
  const onClick = async () => {
    if (!resolvedMarkdownUrl) return
    try {
      let text = cache.get(resolvedMarkdownUrl)
      if (!text) {
        const res = await fetch(resolvedMarkdownUrl)
        text = await res.text()
        cache.set(resolvedMarkdownUrl, text)
      }
      void copy(text)
    } catch (err) {
  ...
  const items = useMemo(() => {
    const fullMarkdownUrl =
      typeof window !== 'undefined' ? new URL(resolvedMarkdownUrl || '', window.location.origin).toString() : 'loading'
    const q = `Read ${fullMarkdownUrl}, I want to ask questions about it.`

    const v0Url = componentName
      ? `https://v0.dev/chat?q=${encodeURIComponent(`Install and integrate the component from https://www.spaceui.one/r/${componentName}.json into my Next.js React project.`)}`
      : `https://v0.dev/chat?q=${encodeURIComponent(`Read the documentation at ${fullMarkdownUrl} and help me build with it.`)}`

    return [
      {
        title: 'View as Markdown',
        href: resolvedMarkdownUrl,
        icon: <IconMarkdown className="size-3.5" />,
      },
      {
        title: 'Open in v0',
        href: v0Url,
  ...
  }, [componentName, githubUrl, markdownUrl])
  ...
  return (
    <Group className="inline-flex items-center rounded-xl bg-muted p-0.5 gap-1 transition-colors">
      ...
      <Button
        ...
        variant="ghost"
        className={cn(
          'inline-flex items-center gap-1.5 px-2.5 bg-background hover:bg-background text-muted-foreground hover:text-foreground',
          checked && 'text-emerald-500 hover:text-emerald-500',
        )}
      >
        {checked ? <IconCheck className="size-3.5 text-emerald-500" /> : <IconCopy className="size-3.5" />}
      ...
      <Popover>
        ...
        <PopoverContent align="end" className="w-52 p-1 bg-popover border border-border rounded-xl">
          ...
          {items.map((item) => (
            <Link
              ...
              target="_blank"
              className="w-full text-sm font-medium px-2.5 py-1.5 rounded-sm inline-flex items-center gap-2 text-foreground/80 hover:text-foreground hover:no-underline hover:bg-muted transition-colors [&_svg]:size-3.5"
            >
...
export { PageActions as ViewOptions, PageActions as LLMCopyButton }
